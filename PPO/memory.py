...
class Memory:
    ...
    def add(
        ...
        self.dones[n_batch_step] = (
            torch.Tensor((int(done is True),)).squeeze(-1).to(self.device)
        )
    ...
    def calculate_gae_advantage(self, values, next_values):
        ...
        for i in reversed(range(self.batch_size)):
            ...
            self.advantages[i] = delta + config.LAMBDA * config.GAMMA * self.advantages[
                i + 1
            ] * (1 - self.dones[i])

            # For critic
            self.gt[i] = self.rewards[i] + config.GAMMA * self.gt[i + 1] * (
                1 - self.dones[i]
            )
