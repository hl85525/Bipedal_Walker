...
class Actor(nn.Module):
    def __init__(
        self, state_space_size, action_space_size, seed, fully_conected_units=256
    ):
...
class Critic(nn.Module):
    def __init__(
        ...
        self.fully_connected_2 = nn.Linear(
            hidden_1_units + action_space_size, hidden_2_units
        )
...
class Memory:
    ...
    def memory_sample(self):
        ...
        dones = (
            torch.from_numpy(
                np.vstack([exp.done for exp in experiences if exp is not None]).astype(
                    np.uint8
                )
            )
            .float()
...
class DDPG_Agent:
    def __init__(self, state_dim, action_dim, random_seed):
        ...
        self.critic_optimizer = optim.Adam(
            self.critic_local.parameters(), lr=3e-4, weight_decay=0.0001
        )

        self.noise = Noise(action_dim, random_seed)

        self.memory = Memory(action_dim, int(1e6), 128, random_seed)
